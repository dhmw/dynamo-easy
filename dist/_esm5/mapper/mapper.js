...
export function toDb(item, modelConstructor) {
    ...
    if (modelConstructor) {
         ...
         */
        if (metadata) {
            metadata.getPropertiesWithDefaultValueProvider().forEach(function (propertyMetadata) {
                var currentVal = Reflect.get(item, propertyMetadata.name);
...
export function toDbOne(propertyValue, propertyPathOrMetadata, propertyMetadata) {
    logger.verbose('map toDbOne', null, { propertyValue: propertyValue, propertyPathOrMetadata: propertyPathOrMetadata, propertyMetadata: propertyMetadata });
...
export function createToKeyFn(modelConstructor) {
    ...
    return function (item) {
        logger.verbose('create key', null, { item: item, propertyMeta: keyProperties });
...
export function fromDb(attributeMap, modelConstructor) {
    logger.verbose('parse fromDb', modelConstructor, { attributeMap: attributeMap });
    var model = {};
...
export function fromDbOne(attributeValue, propertyMetadata) {
    logger.verbose('parse fromDbOne', null, { attributeValue: attributeValue, propertyMetadata: propertyMetadata });
    var explicitType = hasType(propertyMetadata) ? propertyMetadata.typeInfo.type : null;
